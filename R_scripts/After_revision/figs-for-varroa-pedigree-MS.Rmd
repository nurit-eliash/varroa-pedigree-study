...
### Supplementary Figure: setting the threwhols for site quality: Distribution of VAF and QAF in F2 Offspring of homozygous parents
#### QAF
```{r qaf-distribution, echo=FALSE, eval=TRUE}
...
QAF_all_gt <- ggplot(f2_homo_parents, aes(x = QAF, fill = GT)) +
  ...
  geom_vline(xintercept = c(0.03, 0.4, 0.6, 0.8), linetype = "dashed", color = "gray", alpha = 0.5) +
  annotate("rect", xmin = -Inf, xmax = 0.03, ymin = -Inf, ymax = Inf, alpha = 0.4, fill = "#ffbf00") +
  annotate("rect", xmin = 0.4, xmax = 0.6, ymin = -Inf, ymax = Inf, alpha = 0.4, fill = "#66b032") +
  annotate("rect", xmin = 0.8, xmax = Inf, ymin = -Inf, ymax = Inf, alpha = 0.4, fill = "#1982c4")

# QAF: AA genotype
QAF_AA <- ggplot(subset(f2_homo_parents, GT == "AA"), aes(x = QAF, fill = GT)) +
  geom_histogram(binwidth = 0.02, alpha = 1, position = "identity") +
  scale_x_continuous(breaks = seq(0, 1, by = 0.1), limits = c(-0.05, 1.05)) +
  theme_classic() +
  labs(title = "B",
       x = "Quality Allele Frequency (QAF)",
       y = "Count") +
  scale_fill_manual(values = c("AA" = "#ffbf00"), name = "Genotype") +
  theme(
  ...
  geom_vline(xintercept = 0.03, linetype = "dashed", color = "gray", alpha = 0.5) +
  annotate("rect", xmin = -Inf, xmax = 0.03, ymin = -Inf, ymax = Inf, alpha = 0.4, fill = "#ffbf00")

# QAF: AB genotype
QAF_AB <- ggplot(subset(f2_homo_parents, GT == "AB"), aes(x = QAF, fill = GT)) +
  geom_histogram(binwidth = 0.02, alpha = 1, position = "identity") +
  scale_x_continuous(breaks = seq(0, 1, by = 0.1), limits = c(-0.05, 1.05)) +
  theme_classic() +
  labs(title = "C",
       x = "Quality Allele Frequency (QAF)",
       y = "Count") +
  scale_fill_manual(values = c("AB" = "#66b032"), name = "Genotype") +
  theme(
    legend.position = "none",
    axis.text.x = element_text(size = 10, angle = 45, hjust = 1, vjust = 1),
    axis.title.x = element_text(margin = margin(t = 15)),
  ...
  geom_vline(xintercept = c(0.4, 0.6), linetype = "dashed", color = "gray", alpha = 0.5) +
  annotate("rect", xmin = 0.4, xmax = 0.6, ymin = -Inf, ymax = Inf, alpha = 0.4, fill = "#66b032")

# QAF: BB genotype
QAF_BB <- ggplot(subset(f2_homo_parents, GT == "BB"), aes(x = QAF, fill = GT)) +
  geom_histogram(binwidth = 0.02, alpha = 1, position = "identity") +
  scale_x_continuous(breaks = seq(0, 1, by = 0.1), limits = c(-0.05, 1.05)) +
  theme_classic() +
  labs(title = "D",
       x = "Quality Allele Frequency (QAF)",
       y = "Count") +
  scale_fill_manual(values = c("BB" = "#1982c4"), name = "Genotype") +
  theme(
    legend.position = "none",
    axis.text.x = element_text(size = 10, angle = 45, hjust = 1, vjust = 1),
    axis.title.x = element_text(margin = margin(t = 15)),
  ...
  geom_vline(xintercept = 0.8, linetype = "dashed", color = "gray", alpha = 0.5) +
  annotate("rect", xmin = 0.8, xmax = Inf, ymin = -Inf, ymax = Inf, alpha = 0.4, fill = "#1982c4")

# Combine QAF plots with a single legend on the right for plot A only
QAF_combined <- (QAF_all_gt + QAF_AA + QAF_AB + QAF_BB + plot_layout(ncol = 4))

# Display the combined QAF plot
QAF_combined

# Add caption below the plot
cat("**Figure S1.** Distribution of Quality Allele Frequency (QAF) values in F2 females from crosses with homozygous F0 and F1 parents.\nA. Overlaid histogram of QAF values for all genotypes.\nB. Same data faceted by genotype (AA, AB, BB), showing the distribution and spread within each genotype class.\nVertical axis represents count of sites; horizontal axis is QAF. Bin width = 0.02.\nGenotypes are color-coded: AA (yellow), AB (green), BB (blue).")
```

#### VAF
```{r vaf-distribution, echo=FALSE, eval=TRUE}
...
# VAF: AA genotype
VAF_AA <- ggplot(subset(f2_homo_parents, GT == "AA"), aes(x = VAF, fill = GT)) +
  geom_histogram(binwidth = 0.02, alpha = 1, position = "identity") +
  scale_x_continuous(breaks = seq(0, 1, by = 0.1), limits = c(-0.05, 1.05)) +
  theme_classic() +
  labs(title = "B",
...
# VAF: AB genotype
VAF_AB <- ggplot(subset(f2_homo_parents, GT == "AB"), aes(x = VAF, fill = GT)) +
  geom_histogram(binwidth = 0.02, alpha = 1, position = "identity") +
  scale_x_continuous(breaks = seq(0, 1, by = 0.1), limits = c(-0.05, 1.05)) +
  theme_classic() +
  labs(title = "C",
...
# VAF: BB genotype
VAF_BB <- ggplot(subset(f2_homo_parents, GT == "BB"), aes(x = QAF, fill = GT)) +
  geom_histogram(binwidth = 0.02, alpha = 1, position = "identity") +
  scale_x_continuous(breaks = seq(0, 1, by = 0.1), limits = c(-0.05, 1.05)) +
  theme_classic() +
  labs(title = "D",
...
# Combine VAF plots with a single legend on the right for plot A only
VAF_combined <- (VAF_all_gt + VAF_AA + QAF_AB + VAF_BB + plot_layout(ncol = 4))

# Display the combined QAF plot
VAF_combined

# Add caption below the plot
cat("**Figure S2.** Distribution of Variant Allele Frequency (VAF) values in F2 females from crosses with homozygous F0 and F1 parents.\nA. Overlaid histogram of VAF values across all genotypes.\nB. Faceted distribution of VAF by genotype, showing characteristic frequency patterns for AA, AB, and BB.\nVertical axis represents count of sites; horizontal axis is VAF. Bin width = 0.02.\nGenotypes are color-coded: AA (yellow), AB (green), BB (blue).")
```
...
# Add captions below each plot
cat("\n**Figure 1B.** Distribution of genotypes in offspring from crosses between AA males and AB females, showing filtered sites with minimum 40 sites per sample. Top panel shows overall proportions across all chromosomes, while bottom panel shows chromosome-specific distributions. Bar heights represent the proportion of each genotype (AA, AB, BB) in males and females, with sample sizes and total site counts indicated below each bar.")

cat("\n**Figure 1C.** Boxplot distribution of AA genotype proportions across chromosomes in male and female offspring. Each point represents an individual sample, with boxes showing the median and quartile ranges. Data is filtered to include only samples with 40 or more sites. The plot demonstrates sex-specific differences in AA genotype frequency across all seven chromosomes.")
```

# Supplementary Figures for Manuscript

## Variant Quality Distribution (Figure S 6) לתקן כאן את מספר האיור 
...
cat("\n**Figure S3A.** Chromosome count analysis in male and female Varroa destructor mites. Points show mean chromosome counts for individual mites, with error bars representing 95% confidence intervals. Point size indicates the number of cells examined per individual. Blue and pink horizontal lines represent the expected haploid (n=7) and diploid (2n=14) chromosome numbers, respectively. Statistical significance between male and female counts is indicated by the horizontal bar.")
