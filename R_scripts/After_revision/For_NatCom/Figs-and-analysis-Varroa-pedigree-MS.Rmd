...
The full paper by Eliash et al. 2025, "Varroa mites escape the evolutionary trap of haplodiploidy", can be found in [here](https://doi.org/10.1101/2024.05.10.593493).  
...
fm <- rep(0,Tt)
f[1] <- ff[1] <- fm[1] <- f1
f[2] <- P[2]/8*(1+f[1])+f[1]/2+(1-P[2])/4*ff[1]+fm[1]/4
ff[2] <- (P1[3]/4+P2[3]/8+P3[3]/8)*(1+f[1])+f[1]/2+(1-P1[3]-P3[3])/4*ff[1]+(1-P1[3]-P2[3])/4*fm[1]
fm[2] <- P2[3]/(1-P1[3]-P3[3])/8*(1+f[1])+f[1]/2+ff[1]/4+P4[3]/(1-P1[3]-P3[3])/4*fm[1]

for (t in 3:Tt) {
    f[t] <- P[t]/8*(1+f[t-2])+f[t-1]/2+(1-P[t])/4*ff[t-1]+fm[t-1]/4
    ff[t] <- (P1[t]/4+P2[t]/8+P3[t]/8)*(1+f[t-2])+f[t-1]/2+(1-P1[t]-P3[t])/4*ff[t-1]+(1-P1[t]-P2[t])/4*fm[t-1]
    fm[t] <- P2[t]/(1-P1[t]-P3[t])/8*(1+f[t-2])+f[t-1]/2+ff[t-1]/4+P4[t]/(1-P1[t]-P3[t])/4*fm[t-1]
}
p3 <- (1-f)*p0
...
coord_cartesian(xlim = c(4, 31), ylim = c(2, 20)) +
...
cat("\n**Supplementary Figure 1.** Model-predicted decrease in effective
population size (Ne) under haplo-diploidy, diploid arrhenotoky, complete diploidy across two demographic scenarios with constant ratio between carrying capacity (C) and number of brood cells (K). The ratio is set to 2 to match the average number of females co-infest a single brood cell in Varroa4. The number of adult daughters (n) is set to 2 to match the observed average number of adult daughters per female in Varroa5. The Effective population size is calculated from the per-generation change in heterozygosity as N_e=-1/2[(p(t))/(p(t-1))-1] , where p(t) is heterozygosity at generation t. Simulations start with one founding female. Red lines show haplo-diploidy, blue lines show diploid arrhenotoky, and black lines show complete diploidy. Panel a: C = 20, K = 10. At carrying capacity (generation 7), N_e equals 9.01 under haplo-diploidy, 10.73 under diploid arrhenotoky, and 17.19 under complete diploidy. At equilibrium, N_e equals 8.46 under haplo-diploidy, 8.60 under diploid arrhenotoky, and 16.41 under complete diploidy (not plotted). Panel b: C = 200, K = 100. At carrying capacity (generation 12), N_e equals 11.04 under haplo-diploidy, 11.24 under diploid arrhenotoky, and 18.29 under complete diploidy (not plotted). At equilibrium, N_e equals 8.96 under haplo-diploidy, 8.97 under diploid arrhenotoky, and 17.35 under complete diploidy. Dashed vertical lines indicate the first generation at which the modeled population reaches carrying capacity (N=C).")
...
Quality Analysis](https://github.com/nurit-eliash/varroa-pedigree-study/blob/main/R_scripts/After_revision/For_NatCom/site_quality_exploration.Rmd).
...
# Paired t-test
tt <- t.test(df_wide$Female, df_wide$Male, paired = TRUE)

# Paired Cohen's d = mean(diff) / sd(diff)
diffs <- df_wide$Female - df_wide$Male
n <- length(diffs)
d <- mean(diffs) / sd(diffs)

# 95% CI for Cohen's d via noncentral t inversion
t_obs <- as.numeric(tt$statistic)
df <- as.numeric(tt$parameter)
ncp_low <- suppressWarnings(
  uniroot(function(ncp) pt(t_obs, df, ncp) - 0.975, c(-100, 100))$root
)
ncp_high <- suppressWarnings(
  uniroot(function(ncp) pt(t_obs, df, ncp) - 0.025, c(-100, 100))$root
)
d_ci <- c(ncp_low, ncp_high) / sqrt(n)

cat(sprintf(
  "t(%.0f) = %.2f, p = %.3f, Cohen's d = %.2f, 95%% CI [%.2f, %.2f]\n",
  df, t_obs, tt$p.value, d, d_ci[1], d_ci[2]
))
```
