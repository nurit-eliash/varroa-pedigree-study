...
G<-dat %>%
  group_by(sample) %>%
  summarize(G = GTest(as.data.frame(cur_data()))$statistic)
df<-dat %>%
  group_by(sample) %>%
  summarize(df=GTest(as.data.frame(cur_data()))$parameter)
p<-dat %>%
  group_by(sample) %>%
  summarize(pval = GTest(as.data.frame(cur_data()))$p.value)

# total G-value --> sum up df, gvalue --> to get the **total** pvalue
female <- left_join(G, full_join(df, p), by = "sample") %>%
...
# heterogeneity between replicates: 
 dat %>% 
   ...
   dplyr::select(c("obs", "exp")) %>%
    GTest()
 #   not significant, G = 1.5801, X-squared df = 5, p-value = 0.9036
...
pooled_female
 ...
 ## ** 14.5.2022
 צריך להכין טבלה לחישוב 
 g value of pooled :
   על מנת שנוכל לחשב את ה
 pooled results.
``` 
