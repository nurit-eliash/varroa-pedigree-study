...
Assumption: 
1. parthenogenetic males (get genes from mother only). 
2. The 2 alleles in hetero male are distributed equally in the haploid sperm cells.  
...
# plot the median of recombination freq, per chromosome
# filter out families with low number of sites
ggplot(filter(obs_dfAll, total > 30), aes(x=Chr, y=freq_cM_bp, fill=sex)) + 
    geom_boxplot() +
    facet_wrap(~Chr, scale="free") +
    theme_classic() +
    theme(axis.text.x=element_blank()) +
