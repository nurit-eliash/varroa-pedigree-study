...
hist(ad2[,"400_401a_grnson_S112"], breaks = seq(0,1,by=0.02), col = "#1f78b4", xaxt="n")
hist(ad1[,"400_401a_grnson_S112"], breaks = seq(0,1,by=0.02), col = "#a6cee3", add = TRUE)
axis(side=1, at=c(0,0.25,0.333,0.5,0.666,0.75,1), labels=c(0,"1/4","1/3","1/2","1/3","3/4",1))
...
```{r}
adM <- reshape::melt(ad) 
colnames(adM) = c("site", "sample", "gt")
#clean the sample names
adM$sample <- sub("_[^_]+$", "", adM$sample) 

adTable <- adM %>% separate(gt, c("ref", "alt")) %>% 
  na.omit()
# make a table for ploting
table <- adTable %>% 
 dplyr::mutate("ref" = as.numeric(ref)) %>%
 dplyr::mutate("alt" = as.numeric(alt)) %>%
 dplyr::mutate(Ref=ref/(ref+ alt)) %>%   
 dplyr::mutate(Alt=alt/(ref+ alt)) %>%
  dplyr::select(c("site","sample","Ref","Alt")) %>%
  pivot_longer(c("Ref","Alt")) %>%
  dplyr::rename("Allele"="name")

#plot adult males
P_male <- table %>% dplyr::filter(grepl("_son", sample)) %>%
  ggplot(aes(x=value, color=Allele, fill=Allele)) +
    geom_histogram() +
    theme_classic() +
    theme(
    ...
    xlab("Allele balance") +
    ylab("Site count")+
  ggtitle("Males Allele balance")+
  theme(legend.position="bottom")  +
    facet_wrap(~sample, scales = "free") 

P_female <- table %>% dplyr::filter(grepl("_dat|fnd", sample)) %>%
  ggplot(aes(x=value, color=Allele, fill=Allele)) +
    geom_histogram() +
    theme_classic() +
    theme(
    ...
    xlab("Allele balance") +
    ylab("Site count")+
  ggtitle("Females Allele balance")+
  theme(legend.position="bottom")  +
    facet_wrap(~sample, scales = "free") 

P_nymph <- table %>% dplyr::filter(stringr::str_ends(sample, "grn")) %>%
  ggplot(aes(x=value, color=Allele, fill=Allele)) +
    geom_histogram() +
    theme_classic() +
    theme(
